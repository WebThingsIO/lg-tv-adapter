...
function getDataPath(dataDir) {
  if (dataDir) {
    return path.join(dataDir, 'lg-tv-adapter');
  }
  ...
  return path.join(profileDir, 'data', 'lg-tv-adapter');
}

/**
 * Wrapper around gateway's settings database.
 */
class LgTvDatabase extends Database {
  /**
   * Open the database.
   *
  ...
  migrate() {
    return new Promise((resolve, reject) => {
      ...
      const key = 'addons.lg-tv-adapter.pairing-data.%';
      this.conn.all(
